...
async function load(): Promise<boolean> {
    // 加载类型及样式数据
    let status = true;
    await axios.post('/smoke/get_public', { type: '烟雾' }).then((res) => { records.value = res.data.records }).catch((err) => {
...
async function add(id: string | number) {
    await axios.post('/smoke/explore_add', { token: tokenStore.token, id: id, type: '烟雾' }).then(_ => {
    ...
    <div class="flex flex-col w-full h-full">
        ...
        <div class="grid md:grid-cols-4 lg:grid-cols-5 w-full p-1rem gap-4 h-full bg-#f8fafc dark:bg-dark-900">
            ...
            <Card class="grid md:col-span-3 lg:md:col-span-4">
                <template #title>
                    <h1 class="text-2xl font-bold m-0">烟雾图</h1>
                </template>
                <template #content>
                    <div v-if="records.length > 0" class="grid lg:grid-cols-3 sm:grid-cols-1 md:grid-cols-2 gap-4">
                        <div v-for="record in records" :key="record.id" class="py-3 flex flex-col items-center">
                            <a class="max-w-full rounded-lg shadow-sm shadow-indigo-100 grid">
                                <Image :src="record.url" imageClass="max-w-full"
                                    class="h-56 w-full max-w-full object-cover" preview></Image>
                                <div class="p-2 mt-2">
                                    <div @click="router.push('/dashboard/explore/smoke/' + record.id)"
