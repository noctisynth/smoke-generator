...
<template>
    <div class="flex flex-col w-full h-full">
        ...
        <div class="flex flex-col w-full sm:h-30rem md:h-35rem justify-center items-center bg-#f9f9f9 dark:bg-#222">
            <Image :src="smoke?.url" alt="avatar" class="w-20rem h-20rem max-h-full max-w-full rounded" preview>
            </Image>
        </div>
        <Card>
            <template #header>
                <div class="flex justify-between items-center p-2rem mx-6rem">
                    <div class="flex flex-row items-center gap-2">
                        <Avatar :img="userStore.avatar" size="xlarge" class="mr-2"></Avatar>
                        <div class="flex flex-col">
                            <h2 class="text-xl font-bold mb-0">{{ smoke?.name }}</h2>
                            <p class="text-sm text-gray-500">Generated by <span class="font-bold">{{ smoke?.user }}</span></p>
                        </div>
                    </div>
