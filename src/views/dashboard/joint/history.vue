            ...
            <Card class="grid md:col-span-3 lg:md:col-span-4">
                ...
                <template #content>
                    <DataTable class="!overflow-auto" v-model:filters="filters" :value="history_data" dataKey="id"
                        filterDisplay="row" :globalFilterFields="['name', 'date', 'visiable']"
                        ...
                        <Column field="visiable" header="可见性">
                            ...
                            <template #filter="{ filterModel, filterCallback }">
                                <Dropdown v-model="filterModel.value" @change="filterCallback()"
                                    :options="visiable_list" placeholder="选择" :showClear="true">
                                    <template #option="slotProps">
                                        <Tag :value="getVisiable(slotProps.option)" />
                                    </template>
                                </Dropdown>
                            </template>

                            <template #editor="{ data, field }">
                                <Dropdown v-model="data[field]" :options="visiables" optionLabel="label"
